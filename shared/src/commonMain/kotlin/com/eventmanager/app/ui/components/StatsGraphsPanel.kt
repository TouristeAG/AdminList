...
fun StatsGraphsPanel(
    ...
    Column(
        ...
        // Header with title
        Text(
            text = stringResource(Res.string.stats_and_graphs),
...
private fun ActiveVolunteersGraph(
    ...
    GraphCard(
        ...
        yAxisLabel = stringResource(Res.string.count),
        description = stringResource(Res.string.active_volunteers_graph_description),
...
private fun FreeDrinksGraph(
    ...
    GraphCardWithExport(
        ...
        isPhone = isPhone,
        valueFormatter = { it.toInt().toString() },
        yAxisLabel = stringResource(Res.string.free_drinks),
        description = stringResource(Res.string.free_drinks_statistics_description)
    )
}
    ...
    Column(
        ...
        allVenueData.forEach { (venueName, dataPoints) ->
            if (dataPoints.isNotEmpty()) {
                GraphCardWithExport(
                    ...
                    valueFormatter = { it.toInt().toString() },
                    yAxisLabel = stringResource(Res.string.count),
                    description = stringResource(Res.string.venue_shifts_description, venueName)
                )
            }
        }
        ...
        if (totalData.isNotEmpty()) {
            GraphCardWithExport(
                ...
                valueFormatter = { it.toInt().toString() },
                yAxisLabel = stringResource(Res.string.count),
                description = stringResource(Res.string.total_shifts_description)
            )
        }
    }
}
...
private fun GuestListStatisticsGraph(
    ...
    Column(
        ...
        if (volunteerGuestData.isNotEmpty()) {
            GraphCardWithExport(
                title = stringResource(Res.string.volunteers),
                ...
                valueFormatter = { it.toInt().toString() },
                yAxisLabel = stringResource(Res.string.count),
                description = stringResource(Res.string.volunteer_guests_description)
            )
        }
        ...
        if (volunteerInvitesData.isNotEmpty()) {
            GraphCardWithExport(
                title = stringResource(Res.string.invitations),
                ...
                valueFormatter = { it.toInt().toString() },
                yAxisLabel = stringResource(Res.string.count),
                description = stringResource(Res.string.volunteer_invites_description)
            )
        }
        ...
        if (totalGuestData.isNotEmpty()) {
            GraphCardWithExport(
                ...
                valueFormatter = { it.toInt().toString() },
                yAxisLabel = stringResource(Res.string.count),
                description = stringResource(Res.string.total_guests_description)
            )
        }
    }
}
    ...
    if (genderData.totalCount == 0) {
        Card(
            ...
            Box(
                ...
                Text(
                    text = stringResource(Res.string.no_gender_data),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            }
    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    text = stringResource(Res.string.gender_distribution),
                ...
                Column(
                    ...
                    genderData.segments.forEach { segment ->
                        val genderLabel = when (segment.gender) {
                            ...
                            null -> stringResource(Res.string.unspecified)
                        }
            ...
            scope.launch(Dispatchers.IO) {
                try {
                    if (pendingExportType == ExportType.XLSX) {
                        val rows = genderData.segments.map { segment ->
                            val genderLabel = when (segment.gender) {
                                Gender.FEMALE -> getString(Res.string.gender_female)
                                Gender.MALE -> getString(Res.string.gender_male)
                                Gender.NON_BINARY -> getString(Res.string.gender_non_binary)
                                Gender.OTHER -> getString(Res.string.gender_other)
                                Gender.PREFER_NOT_TO_DISCLOSE -> getString(Res.string.gender_prefer_not_to_disclose)
                                null -> getString(Res.string.unspecified)
                            }
                            DistributionExportRow(genderLabel, segment.count, segment.percentage)
                        }
                        val file = GraphExportBridge.exportDistributionXlsx(
                            platformContext = platformContext,
                            fileNamePrefix = "gender_distribution",
                            sheetName = "Gender Distribution",
                            title = getString(Res.string.gender_distribution),
                            firstColumnHeader = "Gender",
                            rows = rows
                        )
                        withContext(Dispatchers.Main) {
                            exportedFile = file
                        ...
                        val file = GraphExportBridge.exportPieChartJpg(
                            platformContext = platformContext,
                            title = getString(Res.string.gender_distribution),
                            segments = segments
                        )
    ...
    if (ageData.totalCount == 0) {
        Card(
            ...
            Box(
                ...
                Text(
                    text = stringResource(Res.string.no_age_data),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            }
    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    text = stringResource(Res.string.age_distribution_title),
                    style = if (isPhone) MaterialTheme.typography.titleMedium else MaterialTheme.typography.titleLarge,
                    fontWeight = FontWeight.SemiBold,
                    color = MaterialTheme.colorScheme.onSurface
                )
                ...
                Column(
                    ...
                    ageData.segments.forEach { segment ->
                        val ageLabel = when (segment.ageRange) {
                            AgeRange.UNDER_18 -> stringResource(Res.string.age_under_18)
                            AgeRange.AGE_18_20 -> stringResource(Res.string.age_18_20)
                            AgeRange.AGE_21_23 -> stringResource(Res.string.age_21_23)
                            AgeRange.AGE_24_26 -> stringResource(Res.string.age_24_26)
                            AgeRange.AGE_27_30 -> stringResource(Res.string.age_27_30)
                            AgeRange.OVER_31 -> stringResource(Res.string.age_over_31)
                            AgeRange.UNKNOWN -> stringResource(Res.string.age_unknown)
                        }
    ...
    LaunchedEffect(isExporting, pendingExportType) {
        if (isExporting && pendingExportType != null) {
            scope.launch(Dispatchers.IO) {
                try {
                    if (pendingExportType == ExportType.XLSX) {
                        val rows = ageData.segments.map { segment ->
                            val ageLabel = when (segment.ageRange) {
                                AgeRange.UNDER_18 -> getString(Res.string.age_under_18)
                                AgeRange.AGE_18_20 -> getString(Res.string.age_18_20)
                                AgeRange.AGE_21_23 -> getString(Res.string.age_21_23)
                                AgeRange.AGE_24_26 -> getString(Res.string.age_24_26)
                                AgeRange.AGE_27_30 -> getString(Res.string.age_27_30)
                                AgeRange.OVER_31 -> getString(Res.string.age_over_31)
                                AgeRange.UNKNOWN -> getString(Res.string.age_unknown)
                            }
                            DistributionExportRow(ageLabel, segment.count, segment.percentage)
                        }
                        val file = GraphExportBridge.exportDistributionXlsx(
                            platformContext = platformContext,
                            fileNamePrefix = "age_distribution",
                            sheetName = "Age Distribution",
                            title = getString(Res.string.age_distribution_title),
                            firstColumnHeader = "Age Range",
                            rows = rows
                        )
                        withContext(Dispatchers.Main) {
                            exportedFile = file
                ...
                modifier = Modifier
                    ...
                    .background(
                        color = MaterialTheme.colorScheme.surfaceVariant.copy(
                            alpha = if (isPhone) 0.15f else 0.35f
                        ),
...
private fun PointerInputScope.updateSingleSeriesInteraction(
    change: PointerInputChange,
    dataPoints: List<DataPoint>,
    onInteractionChange: (InteractionState) -> Unit,
    pressed: Boolean,
) {
    val pointIndex = calculateNearestPointIndex(change.position.x, size.width.toFloat(), dataPoints)
    if (pointIndex >= 0 && pointIndex < dataPoints.size) {
        val dataPoint = dataPoints[pointIndex]
        onInteractionChange(
            InteractionState(
                isPressed = pressed,
...
private fun PointerInputScope.updateMultiSeriesInteraction(
    change: PointerInputChange,
    dataPoints: List<DataPoint>,
    onInteractionChange: (InteractionState) -> Unit,
    pressed: Boolean,
    ...
    if (pointIndex >= 0 && pointIndex < dataPoints.size) {
        onInteractionChange(
            InteractionState(
                ...
                hoveredPointIndex = pointIndex,
                hoveredValue = 0f,
                hoveredLabel = dataPoints[pointIndex].label
            )
        )
    }
}
...
private fun ExportOptionsDialog(
    ...
    GraphExportDialogShell(onDismiss = onDismiss) {
                ...
                Text(
                    text = stringResource(Res.string.choose_export_format),
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
                ...
                Card(
                    ...
                    Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            Text(
                                text = stringResource(Res.string.export_as_xlsx),
                                style = MaterialTheme.typography.titleMedium,
                                fontWeight = FontWeight.SemiBold
                            )
                            Text(
                                text = stringResource(Res.string.export_xlsx_description),
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                ...
                Card(
                    ...
                    Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            Text(
                                text = stringResource(Res.string.export_as_jpg),
                                style = MaterialTheme.typography.titleMedium,
                                fontWeight = FontWeight.SemiBold
                            )
                            Text(
                                text = stringResource(Res.string.export_jpg_description),
                                style = MaterialTheme.typography.bodySmall,
                                color = MaterialTheme.colorScheme.onSurfaceVariant
                            )
                        }
                            ...
                            decodeJpgPreview(file)?.let {
                                Column(
                                    ...
                                    Box(
                                        ...
                                        Image(
                                            bitmap = it,
                                            contentDescription = stringResource(Res.string.graph_preview),
                                            modifier = Modifier
                                                .fillMaxSize()
                                                .padding(12.dp),
                                ...
                                if (isWideLayout) {
                                    ...
                                    Row(
                                        ...
                                        Column(
                                            ...
                                            Text(
                                                text = stringResource(Res.string.xlsx_export_complete),
                                            ...
                                            Text(
                                                text = stringResource(Res.string.xlsx_export_success_message),
                                                style = MaterialTheme.typography.bodyLarge,
                                                color = MaterialTheme.colorScheme.onSurfaceVariant
                                            )
