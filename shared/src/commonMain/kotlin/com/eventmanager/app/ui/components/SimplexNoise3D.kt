...
/**
 * 3D simplex noise (Stefan Gustavson, public domain) — used for posterized topographic edges.
 * Returns values in approximately [-1, 1].
 */
internal object SimplexNoise3D {
    ...
    private val p = IntArray(256) {
        val perm = intArrayOf(
            ...
            74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 211, 229, 122,
    ...
    private val permMod12 = IntArray(512) { p[it and 255] % 12 }
    ...
    fun noise(x: Float, y: Float, z: Float): Float {
        ...
        if (t0 > 0f) {
            val gi = permMod12[ii + perm[jj + perm[kk]]]
            t0 *= t0
        ...
        if (t1 > 0f) {
            val gi = permMod12[ii + i1 + perm[jj + j1 + perm[kk + k1]]]
            t1 *= t1
        ...
        if (t2 > 0f) {
            val gi = permMod12[ii + i2 + perm[jj + j2 + perm[kk + k2]]]
            t2 *= t2
        ...
        if (t3 > 0f) {
            val gi = permMod12[ii + 1 + perm[jj + 1 + perm[kk + 1]]]
            t3 *= t3
