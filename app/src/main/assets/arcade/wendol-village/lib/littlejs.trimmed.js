...
class TextureInfo
{
        ...
        constructor(image)
    {
        ...
        this.glTexture = glContext ? glCreateTexture(image) : null;
    ...
    angle=0, mirror, additiveColor=new Color(0,0,0,0), screenSpace, context)
{
    ...
    if (textureInfo)
    {
        if (!glContext && mainContext)
        {
            const screenPos = worldToScreen(pos);
            const drawSize = size.scale(cameraScale);
            const sx = tileInfo.pos.x;
            const sy = tileInfo.pos.y;
            const sw = tileInfo.size.x;
            const sh = tileInfo.size.y;
            ...
            ctx.drawImage(
                textureInfo.image,
                sx, sy, sw, sh,
                (mirror ? drawSize.x / 2 : -drawSize.x / 2), -drawSize.y / 2,
                mirror ? -drawSize.x : drawSize.x, drawSize.y
            );
            ctx.restore();
            return;
        }
    ...
    else
    {
        if (!glContext && mainContext)
        {
            ...
            ctx.fillRect(-drawSize.x / 2, -drawSize.y / 2, drawSize.x, drawSize.y);
            ctx.restore();
            return;
        }
...
function glInit()
{
    // Android WebView composites a transparent WebGL overlay above the 2D canvas,
    // which hides the map and shows only the page background. Use Canvas2D instead.
    glOverlay = false;
    glCanvas = null;
    glContext = null;
    return;
    ...
    // setup vertex and fragment shaders (WebGL2 / GLSL ES 3.0)
    ...
    // if shaders failed to compile/link, fall back to Canvas2D
    if (!glShader)
    {
        if (glOverlay && glCanvas.parentNode)
...
function glFlush()
{
    if (!glContext || !glInstanceCount) return;
...
function engineInit(gameInit, gameUpdate, gameRenderPost)
{
    ...
    window.__wendolResizeCanvas = updateCanvas;
    
    // create promises for loading images
    const image = new Image;
    image.onerror = function () {
        if (window.__WENDOL_TEXTURE_DATA_URL && image.src !== window.__WENDOL_TEXTURE_DATA_URL) {
            image.src = window.__WENDOL_TEXTURE_DATA_URL;
            return;
        }
        document.body.innerHTML +=
            '<pre style="color:white;padding:16px;font:14px monospace;">' +
            'Wendol Village texture failed to load.</pre>';
    };
    image.onload = () => {
        try {
            if (!image.width || !image.height) {
                throw new Error('Texture image is empty');
            }
            ...
            document.body.innerHTML =
                '<pre style="color:white;padding:16px;font:14px monospace;">' +
                'Wendol Village engine failed:\n' + error.message + '</pre>';
        }
    };
    image.src = window.wendolTextureSrc ? window.wendolTextureSrc() : 't.png';
}
