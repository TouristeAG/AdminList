function scaleCanvas() {
	...
	var height = window.innerHeight || document.documentElement.clientHeight || 600;
	var dpr = window.devicePixelRatio || 1;

	if (height > width) {
		settings.scale = (width / 800) * settings.baseScale;
	} else {
		settings.scale = (height / 800) * settings.baseScale;
	}
...
function setBottomContainer() {
	...
	if (!buttonCont.length || !buttonCont.offset()) {
		return;
	}
...
function set_score_pos() {
	...
	if (!container.length || !buttonCont.length || !igt.length || !container.offset() || !buttonCont.offset() || !igt.offset()) {
		return;
	}
